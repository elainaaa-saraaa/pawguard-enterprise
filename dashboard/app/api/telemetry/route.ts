...
export async function POST(req: Request) {
  ...
  try {
    ...
    // 3. Robust JSON Parsing (Safe against messy Windows/PowerShell encoding strings)
    ...
    try {
      ...
      return NextResponse.json(
        { success: false, error: `JSON Malformed: ${parseError.message}`, received: rawText },
        { status: 400, headers: corsHeaders }
      );
    }

    // 4. Connect to Database & Store
