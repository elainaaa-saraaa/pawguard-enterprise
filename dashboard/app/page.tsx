...
export default function PawGuardDashboard() {
  ...
  // Smart backup mapping so your Python data shows up perfectly in your clean layout
  const hardwareEvent = telemetry?.audio_analytics?.detected_classification || telemetry?.event || 'No Events Broadcasted';
  
  let formattedTime = 'N/A';
  if (telemetry?.timestamp) {
    // If Python sends a Unix timestamp (seconds), convert to milliseconds for JavaScript
    const t = telemetry.timestamp;
    const dateObj = typeof t === 'number' && t < 1000000000000 ? new Date(t * 1000) : new Date(t);
